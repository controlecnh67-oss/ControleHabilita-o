...
export const GoogleDriveBackupCard: React.FC = () => {
  ...
  return (
    <div id="google-drive-backup-section" className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-xs space-y-6">
      ...
      {authError && (
        <div className="p-3.5 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 rounded-xl text-xs text-rose-800 dark:text-rose-300 flex flex-col sm:flex-row sm:items-center justify-between gap-2 font-medium">
          <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setShowConfigModal(true)}
            className="px-2.5 py-1 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-rose-700 dark:text-rose-300 rounded-lg text-[11px] font-bold border border-rose-200 dark:border-rose-800 shrink-0 cursor-pointer"
          >
            Ver Configurações de Origem / Client ID
          </button>
        </div>
      )}
            ...
            <div className="space-y-4 text-xs">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-mono text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 text-xs"
                />
                <p className="text-[11px] text-slate-500">
                  Cole aqui o <strong>ID do cliente OAuth 2.0</strong> criado no Google Cloud Console.
                </p>
              </div>

              {/* Origem JavaScript Autorizada para Copiar */}
              <div className="space-y-1.5 p-3 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-700 dark:text-slate-300">
                    Origem JavaScript Autorizada (Para o Google Console):
                  </span>
                  <button
                    onClick={handleCopyOrigin}
                    className="px-2 py-0.5 bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/60 dark:hover:bg-blue-900/60 text-blue-700 dark:text-blue-300 rounded-md text-[10px] font-bold transition-all flex items-center gap-1 border border-blue-200 dark:border-blue-800 cursor-pointer"
                  >
                    {copiedOrigin ? <Check className="w-3 h-3 text-emerald-500" /> : <Copy className="w-3 h-3" />}
                    <span>{copiedOrigin ? "Copiado!" : "Copiar Origem"}</span>
                  </button>
                </div>
                <div className="p-2 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 font-mono text-[11px] text-slate-700 dark:text-slate-300 break-all select-all">
                  {currentOrigin}
                </div>
                <p className="text-[10px] text-slate-500 dark:text-slate-400">
                  ⚠️ No Google Cloud Console, em <strong>Credenciais ➔ Seu ID do Cliente OAuth</strong>, adicione exatamente esta URL em <strong>"Origens JavaScript autorizadas"</strong> e em <strong>"URIs de redirecionamento autorizados"</strong>.
                </p>
              </div>

              {/* Dica para o erro GeneralOAuthFlow */}
              <div className="p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-xl text-amber-800 dark:text-amber-300 space-y-1">
                <p className="font-bold flex items-center gap-1.5">
                  ...
                  <span>Erro "flowName=GeneralOAuthFlow" ou "Erro 400"?</span>
                </p>
                <ul className="text-[11px] list-disc list-inside space-y-0.5 opacity-90 pl-1">
                  <li>O Tipo do Aplicativo deve ser <strong>"Aplicativo da Web"</strong>.</li>
                  <li>Cole a URL acima no campo <strong>Origens JavaScript autorizadas</strong>.</li>
                  <li>Na <strong>Tela de permissão OAuth</strong>, adicione seu e-mail em <strong>Usuários de teste</strong> (se o app estiver em modo Teste).</li>
                </ul>
              </div>
